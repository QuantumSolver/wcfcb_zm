...
console.log('🚀 WCFCB Budget Check Script Loading...');

frappe.ui.form.on('Material Request', {
    refresh: function(frm) {
        console.log('🔄 Material Request refresh event triggered');

        // Add inline message to confirm script is loading
        frm.dashboard.add_comment(__('✅ Budget Check Script Loaded Successfully!'), 'blue', true);
...
function checkItemBudget(frm, cdt, cdn) {
    ...
    // Enhanced budget check with monthly distribution support
    frappe.call({
        method: 'wcfcb_zm.api.material_request.check_budget',
        args: {
            'expense_account': item.expense_account,
        ...
        callback: function(r) {
            ...
            if (!r.message || r.message.length === 0) {
                frappe.show_alert({
                    message: __('No budget data found for expense account ') + item.expense_account,
                    indicator: 'red'
                });
                return;
            }

            // Show enhanced budget details with monthly distribution
            showEnhancedBudgetDetails(item, r.message, frm);
        },
        error: function(err) {
            frappe.show_alert({
                message: __('Error checking budget'),
...
function showEnhancedBudgetDetails(item, budget_data, frm) {
    const budgetEntry = budget_data[0];
    const requestedAmount = item.amount || 0;

    // Determine budget type and name
    const budgetFor = budgetEntry.budget_against_name || budgetEntry.budget_against;
    const budgetType = budgetEntry.budget_against.includes('PROJ-') ? 'Project' : 'Cost Center';

    // Check overall status
    const isOverAnnualBudget = !budgetEntry.within_annual_budget;
    const isOverMonthlyBudget = budgetEntry.has_monthly_distribution && !budgetEntry.within_monthly_budget;
    const hasMonthlyDistribution = budgetEntry.has_monthly_distribution;

    // Create enhanced dialog
    const dialog = new frappe.ui.Dialog({
        title: __('Enhanced Budget Details'),
        size: 'large',
        fields: [
            {
                fieldtype: 'HTML',
                fieldname: 'budget_content',
                options: generateEnhancedBudgetHTML(item, budgetEntry, {
                    budgetFor,
                    budgetType,
                    requestedAmount,
                    isOverAnnualBudget,
                    isOverMonthlyBudget,
                    hasMonthlyDistribution,
                    budget_data,
...
function generateEnhancedBudgetHTML(item, budgetEntry, options) {
    const {
        budgetFor,
        budgetType,
        requestedAmount,
    ...
    if (isOverMonthlyBudget) {
        overallStatus = 'EXCEEDS MONTHLY BUDGET';
        statusClass = 'badge-warning';
        alertClass = 'alert-warning';
        alertMessage = __('This request exceeds the monthly budget allocation but is within the annual budget.');
    } else if (isOverAnnualBudget) {
        overallStatus = 'EXCEEDS ANNUAL BUDGET';
        statusClass = 'badge-danger';
        alertClass = 'alert-danger';
        alertMessage = __('This request exceeds the annual budget allocation.');
    }

    // Monthly distribution section
    const monthlySection = hasMonthlyDistribution ? `
        <div class="row mb-4">
            <div class="col-12">
                <h6 class="text-primary mb-3">
                    <i class="fa fa-calendar"></i> Monthly Distribution: ${budgetEntry.monthly_distribution_id}
                </h6>
            </div>
            <div class="col-md-3">
                <div class="card border-info">
                    <div class="card-body text-center py-3">
                        <div class="text-muted small">${__('Current Month')}</div>
                        <div class="h5 mb-0 text-info">${budgetEntry.current_month}</div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-info">
                    <div class="card-body text-center py-3">
                        <div class="text-muted small">${__('Monthly Allocation')}</div>
                        <div class="h5 mb-0 text-info">${budgetEntry.monthly_percentage}%</div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-primary">
                    <div class="card-body text-center py-3">
                        <div class="text-muted small">${__('Monthly Budget')}</div>
                        <div class="h5 mb-0 text-primary">${format_currency(budgetEntry.monthly_budget_amount)}</div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card ${isOverMonthlyBudget ? 'border-warning' : 'border-success'}">
                    <div class="card-body text-center py-3">
                        <div class="text-muted small">${__('Monthly Available')}</div>
                        <div class="h5 mb-0 ${isOverMonthlyBudget ? 'text-warning' : 'text-success'}">${format_currency(budgetEntry.monthly_available_budget)}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mb-4">
            <div class="col-12">
                <h6 class="text-muted mb-3">${__('Monthly Breakdown')}</h6>
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-sm mb-0">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="text-right">${__('Percentage of Monthly Budget')}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>${__('Monthly Budget Allocation')}</td>
                                        <td class="text-right font-weight-bold">${format_currency(budgetEntry.monthly_budget_amount)}</td>
                                        <td class="text-right">100%</td>
                                    </tr>
                                    <tr>
                                        <td>${__('Actual Expenses (This Month)')}</td>
                                        <td class="text-right">${format_currency(budgetEntry.monthly_actual_expenses)}</td>
                                        <td class="text-right">${((budgetEntry.monthly_actual_expenses / budgetEntry.monthly_budget_amount) * 100).toFixed(1)}%</td>
                                    </tr>
                                    <tr>
                                        <td>${__('MR Commitments (This Month)')}</td>
                                        <td class="text-right">${format_currency(budgetEntry.monthly_mr_committed)}</td>
                                        <td class="text-right">${((budgetEntry.monthly_mr_committed / budgetEntry.monthly_budget_amount) * 100).toFixed(1)}%</td>
                                    </tr>
                                    <tr>
                                        <td>${__('PO Commitments (This Month)')}</td>
                                        <td class="text-right">${format_currency(budgetEntry.monthly_po_committed)}</td>
                                        <td class="text-right">${((budgetEntry.monthly_po_committed / budgetEntry.monthly_budget_amount) * 100).toFixed(1)}%</td>
                                    </tr>
                                    <tr class="table-active">
                                        <td><strong>${__('Available Monthly Budget')}</strong></td>
                                        <td class="text-right font-weight-bold ${isOverMonthlyBudget ? 'text-warning' : 'text-success'}">${format_currency(budgetEntry.monthly_available_budget)}</td>
                                        <td class="text-right font-weight-bold">${((budgetEntry.monthly_available_budget / budgetEntry.monthly_budget_amount) * 100).toFixed(1)}%</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
    ...
    ` : `
        <div class="alert alert-info mb-4">
            <i class="fa fa-info-circle"></i> <strong>${__('No Monthly Distribution')}</strong><br>
            ${__('This budget does not have monthly distribution configured. The full annual budget is available for any month.')}
        </div>
    `;
    ...
    return `
        <div class="container-fluid p-0">
            <div class="d-flex justify-content-between align-items-start mb-3">
                <div>
                    <div class="h5 mb-1">${__('Budget Details for')} ${item.expense_account}</div>
                    <div class="text-muted small">${budgetType}: ${budgetFor}</div>
                    <div class="text-muted small">${__('Transaction Date')}: ${transaction_date}</div>
                </div>
                <span class="badge ${statusClass} badge-pill">${__(overallStatus)}</span>
            </div>

            ${alertMessage ? `<div class="alert ${alertClass} py-2 mb-3">${alertMessage}</div>` : ''}
                ...
                <div class="col-md-6">
                    <div class="card border-info">
                        <div class="card-body text-center py-3">
                            ...
                            <div class="h4 mb-0 text-info">${format_currency(requestedAmount)}</div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card ${isOverAnnualBudget ? 'border-danger' : 'border-success'}">
                        <div class="card-body text-center py-3">
                            <div class="text-muted small">${__('Available Annual Budget')}</div>
                            <div class="h4 mb-0 ${isOverAnnualBudget ? 'text-danger' : 'text-success'}">${format_currency(budgetEntry.available_budget)}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">
                    <h6 class="text-muted mb-3">${__('Annual Budget Summary')}</h6>
                    <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm mb-0">
                                    <thead class="thead-light">
                                        <tr>
                                            <th>${__('Account')}</th>
                                            <th class="text-right">${__('Annual Budget')}</th>
                                            <th class="text-right">${__('Actual Expenses')}</th>
...
function showBudgetDetails(item, budget_data, monthly_budget_result = null) {
    ...
    if (isLinked) {
        ...
        alertMessage = `<div>
            <div><strong>${__('Budget exceeded for')} ${item.expense_account} – ${contextName}</strong></div>
